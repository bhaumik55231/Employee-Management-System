...
<a href="<%=request.getContextPath()%>/mediaController?flag=viewMediaEmployee&manager_id=<%=manager_id_array[i] %>" ><%=manager_id_array[i] %></a> /	
<%}%><a href="<%=request.getContextPath()%>/mediaController?flag=findMedia2Employee&manager_id=<%=session.getAttribute("userId")%>">Documents uploaded by employee under this Manager</a> / 
<a href="<%=request.getContextPath()%>/mediaController?flag=allPublic">All public Directories</a>  <%if(session.getAttribute("ATE")!=null){ %>/
<a href="<%=request.getContextPath()%>/mediaController?flag=allAteFilesEmployee">View Directory Shared by ATE</a><%} %>
</h4>
</div>
<br><br>
<form action="<%=request.getContextPath()%>/mediaController?flag=uploadMediaEmployee" method="post" enctype="multipart/form-data">
<table align="center">
<tr><td>Select Directory Name*</td><td><select class="form-control" name="director_name"><option disabled="disabled">Select Directory</option><c:forEach items="${requestScope.materiallistEmployee }" var="y"><option value="${y.director_name }">${y.director_name }</option></c:forEach></select></td></tr>
<tr><td><br></td></tr>
...
<tr><td><b>Directory Name</b></td><td><b>Permission Type</b></td><td><b>Files</b></td><td><b>Description</b></td><td><b>Directory Created By</b></td><td><b>File Uploaded By</b></td><td><b>View / Download Document</b></td></tr>
